...
public class LoaderAndSaver {
	...
	public static void loadChests(FileConfiguration config) {
		...
		for (String path : config.getConfigurationSection("party-chests").getKeys(false)) {
			...
			Material mat;
			byte data;
			String region = "", message = "&6Drop Party will start in &e%TIME%&6!", message2 = "§6Drop Party has started!";
			double cost;
			...
			new PartyChest(path, count, mat, data, cost, target, ytarget, dropdelay, dropcooldown, announceinterval, minslots, message, message2, radius, region, enabled, blacklist);
		}
	}
